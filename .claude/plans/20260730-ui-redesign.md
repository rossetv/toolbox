...
- Test: `Tests/ToolboxTests/CompressEngineTests.swift` (extend) AND `Tests/ToolboxTests/CompressEngineMRCTests.swift` (three R7 assertions superseded HERE)
...
- [ ] 1b. **R7-asymmetry reversal (spec §5 change table — an ENGINE behaviour change)**: when a valid hybrid AND a valid gs output both exist, the gate LOSER is retained to `alternateOutput` regardless of which won (today the losing hybrid is discarded). New `CompressEngineMRCTests.testHybridLostGateStillWritesRunnerUp` (hybrid lost ⇒ runner-up file written + descriptor present, kind `.mrc`). Dispositions for the three old R7 assertions: `testHybridLargerThanGsShipsGsOutput`'s no-runner-up assertion SUPERSEDED by the new test; `testHybridSmallerThanGsShipsHybridWithRunnerUp` ADAPTS unchanged (hybrid-won retention already true); the no-gain sibling ADAPTS (withhold-at-≥input still yields no runner-up — nothing valid exists). Run: FAIL → implement → PASS.
...
- [ ] 2. **Banner dismissal (spec §7/§10)**: × dismiss persists PER VERSION — `bannerDismissed` UserDefaults key stores the dismissed version string; the banner never re-shows for that version, shows again for a newer one. Declared in Produces (`UpdateBanner` takes the release + dismissal binding). Tests `testBannerDismissalPersistsPerVersion`, `testNewerVersionReShowsBanner`.
