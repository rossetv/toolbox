...
- [ ] 1. `SelfUpdaterTests` (fixture HTTP server + `hdiutil create` DMGs): checksum mismatch → `.failed`, no swap; mount failure; wrong payload version; `installDestination` nil → degrade; swap failure legs → working install remains, aside named on restore-failure; success path; `testUpdateRefusedWhileBusy` (`isBusy` true → `.blockedByRun`, filesystem untouched); `testNonHTTPSRedirectCancelled` (fixture redirect to `http://` → `.failed`); `testHTTPSRedirectFollowed` (fixture serves the DMG via a 302 to a second local https port — download succeeds; the redirected host is deliberately unconstrained, spec §6.10). Never live GitHub. FAIL → implement → PASS.
