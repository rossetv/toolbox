...
- [ ] 3. Set `baseReduction[.scanColour]` from the measurements (untempered constants chosen so the TEMPERED prediction `base * (0.3 + 0.7 * payloadRatio)` lands within ±15% of measured on both sets; expected neighbourhood `.balanced` ≈ 0.75, `.smallestSize` ≈ 0.80 — the measurements decide). **`.maximumQuality` stays untouched at 0.12 — MRC never runs there (MRC D3), so an MRC-derived constant would predict what the engine cannot deliver.** Fallback if ±15% is unreachable with one constant: widen to ±25%, keep "about" phrasing, and record the achieved tolerance in the plan (edit this step's line with the final constants + tolerance). **DONE — measured and set: the rebuild path gets its own table, `scanColourRebuildReduction = [.balanced: 0.82, .smallestSize: 0.90]`; `baseReduction[.scanColour]` is UNCHANGED (0.12 / 0.45 / 0.65) and now prices only the gs-only path a rebuild opt-out leaves, `.maximumQuality` among it. Achieved tolerance, RELATIVE to each set's median measured reduction: `.smallestSize` **±15%** (worst 9.5%); `.balanced` **±25%** (worst 17.2%) — the fallback was taken because no single `.balanced` constant reaches ±15% on both sets (the synthetic band starts at 0.842, the corpus band ends at 0.834; they miss by 0.008). Cause, measured not assumed: the repo's `.scanColour` fixtures are CGContext raw bitmaps that gs alone crushes 96–99%, so the fixture set is not size-realistic. 0.82 is within 1% of the median of the documents where the hybrid ACTUALLY shipped. Both constants land ABOVE the ≈0.75/0.80 anticipated above — the engine delivers more than the plan expected. Figures in the machine-local calibration record; nothing about the private corpus enters the repo.**
- [ ] 4. The estimator's public entry is re-pinned as `analyse(_ input: URL, mrcEligible: Bool) async -> Analysis` (no default — a defaulted parameter makes the fix inert); eligibility mirrors F2's rule (`(rebuildScan ?? true) && contentType == .scanColour && preset != .maximumQuality`): MRC-class constants only when TRUE, today's constants otherwise (§6.7's honesty). THE VM (this task's second file) passes `effectiveVerbs`-consistent `(overrides[id]?.rebuildScan ?? true)` at every `analyse` call AND re-analyses the row when a `rebuildScan` override changes while idle (the analysis cache is per-file — a flip must re-price; sibling of F4's reservation invalidation). Tests: `testRebuildOptOutRepricesRowEstimate` (QueueAdmissionTests). `EstimatorTests`: `testScanColourPredictionMatchesMRCPipelineOnFixture` (tolerance per step 3), `testScanColourWithRebuildOptOutPredictsNonMRCReduction`, existing fallback tests still green. Commit: `feat(estimate): calibrate scanColour to the MRC path`. **Final constants: recorded in step 3's DONE block above (`scanColourRebuildReduction = [.balanced: 0.82, .smallestSize: 0.90]`; `baseReduction[.scanColour]` unchanged).**
...
**DECIDED IN IMPLEMENTATION (F5e owns these; each was an open question the task text left to the implementer):**
- **Consent on the quality re-run: it FIRES** (`commit` calls the existing `surfaceConsent` once both descriptions are recorded). Spec §7's trigger is a property of a DELIVERY — "shown whenever BOTH a valid hybrid and a valid gs output exist for a rebuilt scan", "as each file's delivery completes, mid-run" — and a re-run builds a NEW pair; the preset selector chooses quality while the sheet chooses how the page looks, so choosing one answers nothing about the other (F5b's carry recommended the opposite on a "the user just chose" reading, which conflates the two axes). `rebuildWithoutAsking` and the `.original`-park exclusion come free from reusing `surfaceConsent`. **The switch re-run (`rerunForSwitch`) deliberately does NOT fire** — the user has just named the variant they want. One entry per row, never per delivery: `surfaceConsent` gained a `contains` guard, because a row whose sheet is still unanswered can now be re-run and the sheet reads the row's CURRENT versions. Tests `testReRunOfARebuiltScanAsksTheConsentQuestionAgain` (both halves) and the `pendingConsents` assertion in `testRerunForSwitchMapsVariantsFromDescriptorKind`.
- **F5a's carry — the armed no-gain row that delivered via its OCR leg**: R11 pins the re-run to the row's existing result path and spec §6.5 says so in the same sentence ("the R11 pin-to-existing-result-path rule continues to govern recompress re-runs"), so the fresh compressed result REPLACES the `-ocr.pdf` file in place (`replaceItemAt` when the destination exists, `moveItem` when it does not) rather than taking a second name. No new reservation, one file per row, and the re-run's own OCR leg keeps it searchable. Test `testReRunOverAnOCROnlyDeliveryReplacesItInPlace`.
- **F5a's carry — shipped bytes**: `commit` now records `outcome.finalBytes` (the post-append re-stat) instead of the compress leg's `after`, matching `ingestCompletedJobs`.
- **A re-run with the read verb OFF DROPS the row's searchability claims** for every file it regenerated, rather than carrying them onto new bytes or manufacturing a `false` (a born-digital input keeps its own text layer through the engine). Spec §6.4 keys "searchable" to the OCR leg's recorded outcome: no leg, no claim. This needed `VersionStore.setSearchable` to accept `Bool?` (nil drops the claim) — a ONE-LINE signature widening outside this task's file list, reported to the orchestrator; every existing `Bool` call site compiles unchanged. Test `testReRunWithOCROffDropsTheStaleSearchabilityClaim`.
- **Copy divergence, owner F5e**: `"That version is no longer available — kept your <preset> version."` — the switch re-run's arm for a regenerated pair that holds no version of the requested kind (the F2 carry: a gs-shipped row whose `.mrc` runner-up vanished used to no-op silently). Composed from two strings already in the queue's register; the handoff has none for this state.
- **Progress channels**: each re-run path publishes its OCR leg through the channel its compress leg already owns — `recompressProgress[id]` for the quality re-run (as the task text says), `rerunProgress[id]` for the switch re-run, whose tail owns that key and whose report closure is guarded on `switchesInFlight`. Both render identically through `publishJobs`. Both paths also now set `legSpans`, so a re-running row's leg label and per-row ETA describe the leg in flight instead of the stale span its last batch left behind (`legLabel`/`rowETASeconds` gate on `.running`, which `publishJobs` synthesises for re-run rows).

- [x] 1. Tests: `testChangeQualityReRunReAppliesOCR`, `testReRunHonoursRowRebuildOptOut`, `testRerunForSwitchMapsVariantsFromDescriptorKind`, `testPreviousSlotCarriesSearchabilityFlag`, `testReRunAppendFailureMarksShippedUnsearchable`, plus the three decision tests above. FAIL → implement → PASS. Full suite: PASS.
- [x] 2. Commit: `feat(queue): re-run paths re-apply OCR and honour row overrides`.
...
**DECIDED IN IMPLEMENTATION (F6 owns these; each was an open question the task text left to the implementer):**
- **`partial` = any row in the batch with `RowOutcome.isDegraded` true** (rescued, tooFaint, cancelled-between-legs, a post-delivery read failure) — delivered, but not a clean full success. The alternative reading considered — "some row never reached a terminal state" (i.e. was left `.queued`/`.analysing` by a cancel) — was rejected: `runnableIDs` already filters `!skippedRows.contains`, so a skipped row is structurally absent from the batch, and `recompress()` always ends terminal (only `cancel()` can requeue a row) — making that reading a strict alias of `cancelled` in this state machine, which is a schema defect (a boolean that always equals another one carries no information). `problem` = any row `.failed` (spec's own "history/problem rows remain the frequency signal for systemic gs failures").
- **Armed (recompress) rows contribute NOTHING to `savedBytes`/`searchableCount`/the cancelled-batch banked check** — every one of those three is scoped to `runQueuedIDs` (this run's freshly-queued rows), never `batchRowIDs` (which also holds armed rows). An armed row's `originalBytes − shipped.bytes` is the row's LIFETIME saving, already folded into `lifetimeSavedBytes` by the batch that first delivered it; counting it again on every Change-Quality re-run would double it. This also fixes a latent "banked" false-positive: an armed row is by definition already `.done` (`recompressState` requires it), so it always carries a non-nil `resultURL` from an EARLIER batch — checking `batchRowIDs` for "banked" would read a cancelled, did-nothing recompress-only pass as having banked something.
- **A pass with no freshly-queued rows at all (`runQueuedIDs` empty — a pure recompress/Change-Quality re-run) records NO history entry**, not a zero-savings one: its own savings are structurally 0 under the rule above, and spec §6.3 already forbids a "0 MB saved" framing. Consequence for P-A/P-B: a Change-Quality pass never appears in Recent batches. Test `testRecompressOnlyBatchDoesNotDoubleCountIntoHistory`.
- **`fileCount` = `batchRowIDs.count`** (every file the pass touched, queued and armed alike) — a display count, not a savings figure, so it is not scoped to `runQueuedIDs` like the three above. A cancelled batch therefore reports every file it *set out* to process, not the number it banked.
- **`folderURL`/`folderName`**: the locked save destination (`lockedRun.folder`) when one was set, else the first queued row's own input folder — mirroring `FileNaming.output`'s own nil-folder resolution (a batch with no destination delivers each file beside itself, so the first file's folder is the only single answer matching what "Open folder" would reveal).
- **`presetTitle`** is nil exactly when the batch's LOCKED `compressOn` is false (read from `lockedRun`, never the live `preset`, which can move during the run).
- **`searchableCount`'s two-path derivation**: a row with a `VersionStore` entry (a compressed row, or the noGain+OCR sibling, both of which DO get recorded) reads `searchableByCard[.shipped]` — the store, never `job.state`, which stays the row's ORIGINAL outcome forever once a recompress has landed. A row with NO store entry at all (the rescue and a verb-off OCR-only delivery — both hit `ingestCompletedJobs`'s `case .skipped, nil: continue` and are never recorded) reads `job.state`'s `outcome.ocr` directly; this is safe specifically because such a row can NEVER re-arm (`recompressState`'s `case nil, .skipped: return .none`), so unlike a compressed row's, its `job.state` can never go stale.
- **Test hermeticity beyond the history tests themselves**: `HeavyEnv` gained a `history`/`historyRoot` seam (temp-directory-backed, alongside its existing `storeRoot`), and the QueueViewModel convenience init (`QueueViewModel(estimator:store:history:)`) gained the same `history:` parameter so `QueueViewModelTests.testThreeFileSyntheticBatchCompressesEndToEnd` — the one pre-existing test that drives a real `compress()` through the bare production entry point — could be pointed at a temp directory instead of silently writing a fake batch into the developer's own `~/Library/Application Support/Toolbox/history.json`.
- **A grown row (`RowOutcome.grew`, spec §6.3 — the OCR append pushed the delivered file past the original) contributes ZERO to `savedBytes`, floored with `max(0, before - after)` per row, never a negative number**: `lifetimeSavedBytes` is a persisted, monotonic "saved since you installed Toolbox" counter, and one grown row in an otherwise-saving batch must never drag it down. Test `testGrownRowContributesZeroNeverNegativeToSavedBytes`.
- **Known limit, not chased**: both `runQueuedIDs` and `batchRowIDs` are snapshotted at `compress()`'s own start, so a file dragged in mid-run (spec §6.5 — it joins the live run) is invisible to this recording pass entirely: its saving never reaches `lifetimeSavedBytes` and it is absent from `fileCount`. This matches `updateBatchProgress`'s own pre-existing `batchRowIDs` snapshot semantics — not a new inconsistency this task introduced — but I1/P-A should know the history figures under-count a batch that grew mid-run.
- **A problem batch (every row `.failed`, nothing cancelled) DOES record an entry with `savedBytes: 0`** — this looks superficially like the recompress-only "0 MB saved" case this task deliberately skips, but the two are not the same: a problem batch has `problem: true`, a true fact worth surfacing (spec §6.9: "history/problem rows remain the frequency signal for systemic gs failures"), whereas a recompress-only pass has no distinguishing fact at all — that absence, not the zero, is what makes it uninteresting. Pinned in `testCompressFailureWithOCROffFailsRow`.

- [x] 1. `HistoryStoreTests`: round-trip, `testClearListPreservesLifetime`, trim at 200, day grouping, corrupt/foreign-version → empty. FAIL → implement → PASS.
- [x] 2. `QueuePassTests`: `testBatchEndRecordsHistory`, `testCancelledBatchWithBankedFileRecordsEntry`, `testCancelledEmptyBatchRecordsNothing`, `testOCROnlyAndRescuedRowsExcludedFromSavedBytes` (history `savedBytes` sums compressed rows only; rescued/noGain+OCR count via `searchableCount`), plus `testRecompressOnlyBatchDoesNotDoubleCountIntoHistory` above. FAIL → wire → PASS.
- [x] 3. Commit: `feat(history): recent-batches store with lifetime savings`.
