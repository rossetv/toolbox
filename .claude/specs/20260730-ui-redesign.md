...
- **README.md edits**: the human ruled the existing claims remain true (update is user-initiated: "you always choose"). That ruling is recorded as covering the full sentence including "That check is the app's only network request" — read as describing unprompted activity; the user-clicked update download sits outside it. Flagged in the approval summary so the ruling is taken with both clauses visible. README is not touched. KB docs and `UpdateChecker` doc comments (Claude-owned) ARE updated to describe the new download path.
...
- **Verb set**: `@Published var compressOn: Bool = true`, `ocrOn: Bool = false` (batch-level), plus `OCROptions` (language, accuracy) and the existing `preset`. At least one verb on ⇒ Start enabled; zero verbs ⇒ Start disabled (chip UI enforces). The same floor holds per row: a per-file toggle that would empty the row's *effective* verb set disables with a caption — no row can reach "nothing to do" through overrides.
...
**DESIGN.md rewrite (D4)**: rewritten from the handoff as the app's visual law. Constraints: **every** in-code reference to DESIGN.md — `§N` forms (14) and prose mentions (42 total: 41 in `Sources/`, 1 in `scripts/make-app-icon.swift`) — is re-anchored or re-worded against the new document in the same change (the sweep greps for `DESIGN.md` **repo-wide outside `.claude/`**, not only the `§N` pattern; zero stale references at gate time); the **Focus (Accessibility) row is preserved** — the stray-focus-ring standing invariant (memory 2026-07-25) anchors to it; `.clearsClickFocus()` and the first-responder-clear behaviour remain mandatory on every `.plain`-styled control the redesign adds.
...
- **Certify (full read, Fable 5): SHIP** — no critical/major; five minors, all fixed: `EngineVariant` three-case parenthetical (stale two-case residue inside the widening sentence), per-row zero-verb floor pinned, §3's README ruling extended to the "only network request" clause + approval-summary flag, DESIGN.md reference count corrected to 42 with repo-wide sweep scope, footer completed. Lesson-candidates: grep for pre-widening case-enumerations inside widening sentences; every override surface needs an "all off" disposition; a ruling over a multi-clause claim must cover every clause.
- **R3 (incremental, Fable 5): SHIP** — F2, m9, m10 confirmed fixed; no new findings. Lesson-candidates: a qualified invariant must be re-qualified at every citation; empirical tests of path-dependent behaviour must state the setup that forces the non-degrade path.
