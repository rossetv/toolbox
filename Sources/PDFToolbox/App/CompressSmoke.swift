...
/// smoke (plan S.5). Triggered by `PDFTOOLBOX_SMOKE=compress`; prints a one-line result and
/// exits. The input is synthetic, in the app's temp dir (never a TCC-scoped user folder).
enum CompressSmoke {
    static func runIfRequested() {
        guard ProcessInfo.processInfo.environment["PDFTOOLBOX_SMOKE"] == "compress" else { return }
    ...
    private static func syntheticImagePDF() throws -> URL {
        ...
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("pdftoolbox-smoke-\(UUID().uuidString)", isDirectory: true)
